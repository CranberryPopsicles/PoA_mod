...
static func check(unit, residence):
	...
	if unit == Global.Player:
		if StateWorld.type == "path":
			ToolMessageCreator.add_message("[color=#c0c0c0]", "你走上道路...")
		
		elif StateWorld.Floor_Current <= 1:
			ToolMessageCreator.add_message("[color=#c0c0c0]", "你进入 " + StateWorld.tileset.description + "...")
...
static func check_effects(unit, residence):
	...
	if unit_traits.has("Apophis"):
		...
		Global.Player.HP += 25
		ToolMessageCreator.add_message("[color=#a0c0c0]", "赞美 [color=#700090]Apophis[/color]！你的 [color=#ff7050]力量[/color] 提升了！[color=#707070]+25 生命[/color]")
		if ToolSettings.settings_data.cycle_current > 1:
