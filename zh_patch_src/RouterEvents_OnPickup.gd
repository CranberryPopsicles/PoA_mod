...
static func transformed(item):
	...
	if Global.Player.get_traits().has("Mardok"):
		if item.type == "armor":
			if item.position == "head":
				item.rumor = item.name + " 被 Mordok 转化..."
	...
	if Global.Player.get_traits().has("Dumuzi"):
		if item.type == "armor":
			if item.weight > 1:
				...
				item.rumor = item.name + " 被 Dumuzi 转化..."
				match item.position:
					"head":
	...
	if Global.Player.get_traits().has("Virya"):
		if item.type == "weapon":
			if item.dmgtype == "slash":
				is_transformed = true
				item.rumor = item.name + " 被 Vyranism 重铸..."
	...
	if Global.Player.get_traits().has("Pallas"):
		if item.type == "weapon":
			if item.shield == true:
				is_transformed = true
				item.rumor = item.name + " 被 Pallas 转化..."
	...
	if Global.Player.get_traits().has("Ihra"):
		if item.type == "armor":
			if item.position == "leg":
				...
				item.rumor = item.name + " 被你的 Sublime Body 转化..."
	...
	if Global.Player.get_traits().has("Oros"):
		if item.type == "weapon":
			is_transformed = true
			item. class = "oros"
			item.rumor = item.name + " 被 Oros 转化..."
...
static func virya_starting(item):
	...
	if Global.Player.get_traits().has("Virya"):
		if item.type == "weapon":
				...
				is_transformed = true
				item.rumor = item.name + " 被 [color=#c03090]Viryanism[/color] 重铸..."
				item.name = "[color=#c03090]Mighty Blade[/color] [color=#707070](" + textstrip.strip_bbcode(item.name) + ")[/color]"
				item.sprite = "res://Ham_Sprite/Weapons/MightyBlade.png"
				item.proj_art = "none"
				item.range = 1
