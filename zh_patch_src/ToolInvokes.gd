...
static func display_trigger(label):
	var labels = {
		"adjacent ally damaged by enemy": "相邻盟友受到敌人伤害",
		"adjacent attack": "相邻攻击",
		"adjacent enemy": "邻近敌人",
		"ally death": "盟友死亡",
		"ant kill": "蚁类击杀",
		"attack bare fist": "空手攻击",
		"attack": "攻击",
		"attacked adjacent": "被相邻攻击",
		"being hit": "受到命中",
		"block": "格挡",
		"damaged by enemy": "受到敌人伤害",
		"divine intervention": "神力介入",
		"dodge": "闪避",
		"dreaming": "梦境",
		"dudar step": "腐蚀敌人移动",
		"enemy death adjacent": "相邻敌人死亡",
		"enemy death in range": "范围内敌人死亡",
		"enemy death": "敌人死亡",
		"game turn none adjacent": "无相邻敌人的游戏回合",
		"gilded": "镀金",
		"glory": "荣耀",
		"heal": "治疗",
		"kill boss": "击杀首领",
		"kill non adjacent": "击杀非相邻敌人",
		"kill on grass": "在草地击杀",
		"kill while bloodrage": "血怒中击杀",
		"kill": "击杀",
		"new area injured": "受伤进入新区",
		"new area uninjured": "无伤进入新区",
		"new area": "进入新区",
		"no adjacent enemy": "没有相邻敌人",
		"pickup": "拾取",
		"stand still adjacent": "贴近敌人原地等待",
		"stand still": "原地等待",
		"step adjacent": "贴近敌人移动",
		"step not adjacent": "远离敌人移动",
		"step": "移动",
		"take 15 damage": "承受 15 点伤害",
		"teleport": "传送",
		"transform item": "转化物品"
	}
	if labels.has(label):
		...
		if dict[invoke].use_recharge_type == label:
			if dict[invoke].level_required <= Global.Player.level:
				...
				if dict[invoke].use < dict[invoke].use_max:
					...
					ToolMessageCreator.add_message("[color=#707070]", textstrip.strip_bbcode(dict[invoke].name) + " +" + str(int(dict[invoke].use_gain)) + "  [color=#707070]" + display_trigger(label) + "[/color]")
				if dict[invoke].use > dict[invoke].use_max:
	...
	for key in dict:
		...
		if invoke.use_lose == label:
			if invoke.level_required <= Global.Player.level:
				...
				if invoke.use > 0:
					ToolMessageCreator.add_message("[color=#707070]", textstrip.strip_bbcode(invoke.name) + " -1" + "  [color=#707070]" + display_trigger(label) + "[/color]")
					invoke.use -= 1
...
static func force_lose_charge(invoke_key, msg):
	...
	if invoke.level_required <= Global.Player.level:
				...
				if invoke.use > 0:
					ToolMessageCreator.add_message("[color=#707070]", textstrip.strip_bbcode(invoke.name) + " -1" + "  [color=#707070]" + display_trigger(textstrip.strip_bbcode(msg)) + "[/color]")
					invoke.use -= 1
...
static func cast(invoke, label):
	ToolMessageCreator.add_message("[color=#c0c0c0]", "[color=#ffff50]你[/color] 诵念了 " + invoke.name)
...
static func tekton():
	...
	if item != null:
		...
		var stringa = ""
		stringa += item.name
		stringa = "[color=#ff7070]Pallas[/color] 强化了你的 " + stringa + " [color=#ff8030]+30 命中[/color] / [color=#5050ff]+30 格挡[/color]"
		stringa += " [color=#707070]提升至 " + str(item.dmg * 10) + " 命中 / " + str(item.arm) + " 格挡 <- Tekton[/color]"
...
static func cheren():
	...
	Global.Player.HP += 20
	ToolMessageCreator.add_message("[color=#a0c0c0]", "伟大的 [color=#80af00]Takhal[/color]！[color=#707070]+20 生命[/color]")
	if ToolSettings.settings_data.cycle_current > 1:
				...
				ToolMessageCreator.add_message("[color=#ff5050]", "轮回令你焕发生机...[color=#707070] +" + str(int(hp_bonus)) + " 生命[/color]")
