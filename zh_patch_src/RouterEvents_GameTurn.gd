...
static func run():
	...
	ToolMessageCreator.add_message("[color=#505050]", "一个游戏回合过去了...")
			...
			if buff.harmful == true:
				...
				if rng.randi_range(1, 100) < unit.type.size:
					...
					"msg": "自然恢复"
						...
						ProcessQueue.add_effect(action)
						ToolMessageCreator.add_message("[color=#707070]", unit.get_name_color() + " 从 " + buff.color + buff.name + "[/color] 中[color=#9010cf]恢复[/color]")
