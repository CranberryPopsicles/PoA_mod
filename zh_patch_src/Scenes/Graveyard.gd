...
func update_title():
	if graveyard_var.EXISTS == false:
		$Control / RichTextLabel.bbcode_text = "[color=#c0c0c0]尚无人走上这条道路...[/color]"
		...
		$Control / RichTextLabel.bbcode_text = "[color=#c0c0c0]朝圣者啊，在你的荣耀中安息...[/color]"
		var count = 0
...
func write_description(data):
	...
	stringa = ""
	if data.has("dust"):
		stringa = "[color=#a0a0a0]已抵达 [color=#a08060]Dust[/color] [color=#ffff50]" + str(data.dust) + "[/color]"
		
	elif data.has("loadable"):
		stringa = "[color=#a0a0a0]踏上 [color=#a08060]Path of Dust[/color]..."
	else:
		stringa = "[color=#a08080]古老的死者啊..."
...
func get_character_description(data):
	...
	stringa += "，" + data.title_race + "\n" + get_base_classe(data) + " 信奉 " + data.title_god
	stringa += "\n\n"
	
	stringa += "[color=#ffa000]荣耀 " + str(int(data.level)) + "[/color]"
	
	stringa += "\n\n"
	
	stringa = stringa + "[color=#ff8080]生命[/color] " + "[color=#ffff50]" + str(int(data.HP_max)) + "[/color]"
	stringa = stringa + "  [color=#20ff20]速度[/color] " + "[color=#ffff50]" + str(int(data.SPEED + (data.DEX * 2))) + "[/color]"
	stringa = stringa + "\n\n[color=#ff7050]STR[/color] " + "[color=#ffff50]" + str(int(data.STR)) + "[/color]"
	stringa = stringa + "  [color=#50ff70]DEX[/color] " + "[color=#ffff50]" + str(int(data.DEX)) + "[/color]"
	stringa = stringa + "  [color=#c050ff]WIL[/color] " + "[color=#ffff50]" + str(int(data.WIL)) + "[/color]"
...
func write_death(data):
	...
	if data.condition == "death":
		...
		stringa += "[color=#ff3000]已胜利！[/color]"
...
func update_highlight():
	...
	for button in buttons:
		...
		if $TextEdit.text != "" and $TextEdit.text != "search...":
