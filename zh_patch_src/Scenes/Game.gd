...
func level_cleared():
	...
	if floor_cleared == false:
		...
		if StateWorld.type != "path":
			ToolMessageCreator.add_message(txcolor, "已肃清！")
		...
		for buff in Player.Buffs:
			if buff.name == "Entangle":
				...
				ToolMessageCreator.add_message("[color=#707070]", "[color=#60af20]缠绕藤蔓[/color]开始脱落...")
			...
			if buff.name == "Bleed":
				...
				ProcessQueue.add_effect(action)
				ToolMessageCreator.add_message("[color=#707070]", "[color=#ff1010]流血[/color]开始止住...")
