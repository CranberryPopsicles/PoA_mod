...
func check_intervention():
	...
	if has_intervened == true:
		...
		ToolMessageCreator.add_message("[color=#ff0000]", "[color=#78bca4]神圣干预[/color]！" + god.name + " 保全了你的生命！..." + invoke.name + "[color=#ff5050] -" + str(int(invoke.use)) + " 充能[/color]")
		invoke.use = 0
		for n in 10:
...
func level_up():
	...
	ToolMessageCreator.add_message(txcolor, "赞美 " + Global.Player.god.name + "！你的荣耀提升了！")
...
func get_killer_name():
	var stringa = "[color=#ff0000]mistake[/color]"
	return stringa
