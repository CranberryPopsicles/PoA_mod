...
func _on_ButtonCharacter_pressed():
	ProcessText.spawn_text_popup_context(Vector2(300, 370), "已删除当前角色存档", "[color=#ff5050]", self)
	saveload.saveData(saveload.create_empty_file())


func _on_ButtonData_pressed():
	ProcessText.spawn_text_popup_context(Vector2(300, 370), "已删除全部数据", "[color=#ff5050]", self)
