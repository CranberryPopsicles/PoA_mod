...
func update_selected():
	...
	if is_loaded == true:
		...
		$Continue / Label.bbcode_text = "[color=#a05050]清除[color=#ffff50]存[/color]档[/color]"
		$Button_Start / Label.text = "继续"
	else:
		$Continue / Label.bbcode_text = "[color=#50a050]读取[color=#ffff50]存[/color]档[/color]"
		$Button_Start / Label.text = "开始"
	update_highlight()
	set_player_sheet()
	draw_body()
	write_description()
...
func write_description():
	...
	if hovered_button == null:
		...
		if hovered_button == $bodybuttons / leg:
			...
			is_weapon = false
		stringa = "[color=#707070]" + stringa + "空置...[/color]"
		if item != null:
			if is_weapon == true:
		...
		stringa += "进入[color=#ffff50]Achra 之地[/color]..."
		...
		stringa += "查看旅途中可能遭遇的敌人..."
	
	elif hovered_button == $Powers:
		
		stringa += "查看旅途中可选择的能力与进阶职业..."
		...
		if is_loaded == false:
				...
				if trait.life + trait.speed > 0:
					stringa += "\n\n"
				if trait.life > 0:
					stringa += "[color=#ff8080]生命[/color] " + "[color=#ffff50]+" + str(int(trait.life)) + "[/color]  "
				if trait.speed > 0:
					stringa += "[color=#20ff20]速度[/color] " + "[color=#ffff50]+" + str(int(trait.speed)) + "[/color]"
	...
	stringa = "[color=#c0c0c0][color=#ffff50]" + StatePlayerSheet.title_name + "[/color]，" + StatePlayerSheet.title_race
	
	
	if is_loaded == true:
		stringa += " [color=#707070](已载入)[/color]"
...
func write_description_hovered():
	...
	if hovered_button.trait.life > 0:
		stringa += "[color=#ff8080]生命[/color] " + "[color=#ffff50]+" + str(int(hovered_button.trait.life)) + "[/color]  "
	if hovered_button.trait.speed > 0:
		stringa += "[color=#20ff20]速度[/color] " + "[color=#ffff50]+" + str(int(hovered_button.trait.speed)) + "[/color]"
...
func describe_armor(armor):
	...
	if armor != null:
		...
		if armor.inflex > 0:
			stringa = stringa + "\n[color=#ff0000]" + "+" + str(armor.inflex) + " [color=#a0a0a0]Inflexibility[/color][/color]"
			
		else:
			stringa = stringa + "\n[color=#50ff50]" + str(armor.inflex) + "[/color] [color=#a0a0a0]Inflexibility[/color]"
...
func update_highlight():
	if $TextEdit.text != "" and $TextEdit.text != "搜索...":
