...
func write_description_date():
	var stringa = ""
	stringa += "[right][color=#c0c0c0]第 " + str(StateWorld.day) + " 天"
	if StateWorld.land == "void":
		stringa += "\n闪烁的[color=#9000a0]虚空[/color]"
	else:
		stringa += "\n[color=#ffff50]Achra[/color] 之地"
	
	if StateWorld.victorious == false:
		stringa += "\n" + cycler.int_to_cycle_name(ToolSettings.settings_data.cycle_current)
	else:
		stringa += "\n[color=#ff9020]胜利！[/color]"
	return stringa
...
func write_description():
	...
	if current_tile != selected_tile:
		stringa += "你凝视着 " + selected_tile.Name + "..."
...
func spread_ocean():
	...
	for tile in tile_list:
		if tile.x <= x and tile != current_tile and tile.data.type != "obelisk" and tile.data.type != "astrolith":
			...
			tile.create()
	if StateWorld.land == "void":
		ToolMessageCreator.add_message("[color=#c0c0c0]", "[color=#9000a0]闪烁的黑暗[/color]升起...")
	else:
		ToolMessageCreator.add_message("[color=#c0c0c0]", "[color=#0000ff]海洋[/color]升起...")
