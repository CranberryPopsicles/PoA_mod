...
func _ready():
	...
	if Global.bestiary_selected != "none":
		$RichTextLabel3.bbcode_text = "[color=#c0c0c0]可见敌人..."
	create_buttons()
...
func write_unit(data):
	...
	if data.cycle.has("hp"): stringa += " [color=#af40af]+ 轮回[/color]"
	
	stringa += "\n"
	stringa += "     "
	stringa += "[color=#20ff20]" + str(data.speed) + "[/color] 速度，最低 " + str(data.speedmin)
	if data.cycle.has("speed"): stringa += " [color=#af40af]+ 轮回[/color]"
	...
	if data.cycle.has("accuracy"): stringa += " [color=#af40af]+ 轮回[/color]"
	
	
	
	stringa += "\n"
	stringa += "     "
	stringa += "[color=#ff8030]" + str(data.damage[0] * 10) + "[/color] 伤害"
	if data.cycle.has("hit"): stringa += " [color=#af40af]+ 轮回[/color]"
	
	stringa += "\n"
	stringa += "     "
	stringa += "[color=#50ffff]" + str(data.dodge[0] * 10) + "[/color] 闪避"
	if data.cycle.has("dodge"): stringa += " [color=#af40af]+ 轮回[/color]"
	
	stringa += "\n"
	stringa += "     "
	stringa += "[color=#5050ff]" + str(data.deflect[2]) + "[/color] 格挡"
	if data.cycle.has("block"): stringa += " [color=#af40af]+ 轮回[/color]"
...
func translate_tier(tier, boss):
	var stringa = "[color=#707070]...游荡于"
	if boss == true:
		if tier < 5:
			stringa = "[color=#707070]...栖居于[/color]"
		else:
			stringa = "[color=#707070]...统治着[/color]"
	match str(tier):
		"1":
			if boss == true: stringa += "[color=#00ff70]绿塔[/color]"
			else: stringa += "[color=#00ff70]早期[/color]地区"
		"2":
			if boss == true: stringa += "[color=#8000af]紫塔[/color]"
			else: stringa += "[color=#8000af]中期[/color]地区"
		"3":
			if boss == true: stringa += "[color=#ff5030]红塔[/color]"
			else: stringa += "[color=#ff5030]后期[/color]地区"
		"4":
			if boss == true: stringa += "[color=#ff90a0]方尖碑[/color]"
			else: stringa += "[color=#ff90a0]最终[/color]地区"
		"5":
			if boss == true: stringa += "[color=#8030ff]星石[/color]"
			else: stringa += "[color=#9000a0]虚空[/color]"
		"6":
			if boss == true: stringa += "[color=#ffff80]心灵巢穴[/color]"
			else: stringa += "[color=#ffff80]心灵巢穴[/color]"
		"7":
			if boss == true: stringa += "[color=#ff2020]血之虚空[/color]"
			else: stringa += "[color=#ff2020]血之虚空[/color]"
		"8":
			if boss == true: stringa += "[color=#30af50]禁行之路[/color]"
			else: stringa += "[color=#30af50]禁行之路[/color]"
		"9":
			if boss == true: stringa += "[color=#5070ff]蓝塔[/color]"
			else: stringa += "[color=#5070ff]蓝塔[/color]"
		"10":
			if boss == true: stringa += "[color=#ff00a0]星界沼泽[/color]"
			else: stringa += "[color=#ff00a0]星界沼泽[/color]"
		"11":
			if boss == true: stringa += "[color=#B0E0E6]被吞噬之星[/color]"
			else: stringa += "[color=#B0E0E6]被吞噬之星[/color]"
		
		"99":
			stringa = "[color=#a070ff]召唤物[/color]"
