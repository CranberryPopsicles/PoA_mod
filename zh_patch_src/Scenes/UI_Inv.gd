...
func search_is_empty(text):
	return text == "" or text == "search bag..." or text == "搜索背包..."
...
func highlight_focused():
	...
	DestroyButton.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]3[/color]] [color=#ff4000]献祭[/color]"
	...
	for button in buttons_bag:
		...
		if hovered_button == str(button.bag_index) + "bag":
			...
			EquipButton.get_node("label").bbcode_text = "装备"
...
func highlight_bag():
	$Description_Bag.bbcode_text = "[color=#9f6040]背包[/color]"
	...
	for button in buttons_bag:
		...
		if search_is_empty($TextEdit.text) == false:
	...
	match hovered_button:
		...
		"main":
			...
			if player.weapon_main != null:
				...
				EquipButton.modulate = Color(1, 1, 1, 1)
				EquipButton.mouse_filter = Control.MOUSE_FILTER_STOP
				EquipButton.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]1[/color]] 放入背包[/color]"
				
			var EquipButton = $PopupLayer / Popup / Equip2
			EquipButton.modulate = Color(1, 1, 1, 1)
			EquipButton.mouse_filter = Control.MOUSE_FILTER_STOP
			EquipButton.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]2[/color]] 交换手位"
		...
		"off":
			...
			if player.weapon_off != null:
				...
				EquipButton.modulate = Color(1, 1, 1, 1)
				EquipButton.mouse_filter = Control.MOUSE_FILTER_STOP
				EquipButton.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]1[/color]] 放入背包"
		...
		"head":
			...
			if player.armor_head != null:
				...
				stringa = "[color=#c0c0c0]头部未装备[/color]"
		...
		"chest":
			...
			if player.armor_chest != null:
				...
				stringa = "[color=#c0c0c0]胸甲未装备[/color]"
		...
		"hand":
			...
			if player.armor_hands != null:
				...
				stringa = "[color=#c0c0c0]手臂未装备[/color]"
		...
		"leg":
			...
			if player.armor_legs != null:
				...
				EquipButton.modulate = Color(1, 1, 1, 1)
				EquipButton.mouse_filter = Control.MOUSE_FILTER_STOP
				EquipButton.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]1[/color]] [color=#c0c0c0]放入背包[/color]"
			else:
				stringa = "[color=#c0c0c0]腿部未装备[/color]"
		...
		if hovered_button == str(button.bag_index) + "bag":
			...
			match item.type:
					"weapon":
						...
						EquipButton.modulate = Color(1, 1, 1, 1)
						EquipButton.mouse_filter = Control.MOUSE_FILTER_STOP
						EquipButton.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]2[/color]] [color=#c0c0c0]装备到副手[/color]"
						$PopupLayer / Popup / Equip.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]1[/color]] 装备到主手[/color]"
					"armor":
						stringa = describe_armor(item)
						$PopupLayer / Popup / Equip.get_node("label").bbcode_text = "[color=#a0a0a0][[color=#ffff50]1[/color]] 穿戴护甲[/color]"
		...
		if hovered_button == button.trait.title:
			...
			if button.trait.generic == true:
				if button.trait.organize == "racial":
					stringa += "[color=#808080]文化[/color]\n"
				elif button.trait.organize == "class":
					stringa += "[color=#808080]职业[/color]\n"
				elif button.trait.organize == "god":
					stringa += "[color=#808080]信仰[/color]\n"
				elif button.trait.organize == "prestige":
					stringa += "[color=#808080]进阶职业[/color]\n"
				else:
					stringa += "[color=#808080]物品[/color]\n"
...
func describe_weapon(weapon):
	...
	if weapon != null:
		...
		stringa += " [color=#a0a0a0]精准[/color]"
		stringa += "\n[color=#ff8030]" + "10" + "[/color]"
		stringa += " [color=#a0a0a0]命中[/color] [color=#af8f50]钝击[/color]"
	...
	stringa = stringa + "\n" + range_string + "[/color] [color=#a0a0a0]射程[/color] "
...
func describe_total_info():
	...
	if player.weapon_main != null and player.weapon_off == null:
		$InfoButtons / States.bbcode_text += "[color=#ffff50]双手持握[/color]"
	else:
		$InfoButtons / States.bbcode_text += "[color=#50ffff]单手持握[/color]"
	if int(game.Player.get_total_weight()) > int(game.Player.get_total_STR()):
		$InfoButtons / States.bbcode_text += "[color=#707070]，[/color] [color=#ff0000]超重[/color]"
	else:
		$InfoButtons / States.bbcode_text += "[color=#707070]，[/color] [color=#00ff00]未超重[/color]"
	if int(game.Player.get_total_inflex()) > 1:
		$InfoButtons / States.bbcode_text += "[color=#707070]，[/color] [color=#ff0000]僵硬[/color]"
	else:
		$InfoButtons / States.bbcode_text += "[color=#707070]，[/color] [color=#00ff00]灵活[/color]"
...
func describe_states():
	var stringa = "\n\n[color=#707070]"
	if player.weapon_main != null and player.weapon_off == null:
		stringa += "[color=#ffff50]双手持握[/color]"
		stringa += "：副手为空并支援主手，增加命中和精准"
	else:
...
func press_equipment(string, item, button):
	...
	if item != null and Global.Player.bag.size() < Global.bagsize and focused_button == string and game.floor_cleared == true:
		...
		ProcessText.spawn_text_popup_context($PopupLayer / Popup.position, "背包已满！", "[color=#ff2020]", $PopupLayer)
	elif game.floor_cleared == false and focused_button == string:
		ProcessText.spawn_text_popup_context($PopupLayer / Popup.position, "敌人就在附近！", "[color=#ff2020]", $PopupLayer)
...
func sacrifice_compose_upgrades(item):
	...
	var stringa = "[color=#ff4000]献祭！[/color]你"
	...
	if is_equipped == true:
		...
		if is_armor == false:
			...
			if acc_amount < 1: acc_amount = 1
			dict.acc += acc_amount
			stringa += " [color=#ffa050]+" + str(acc_amount * 10) + " 精准"
			...
			if hit_amount < 1: hit_amount = 1
			dict.dmg += hit_amount
			stringa += " [color=#ff8030]+" + str(hit_amount * 10) + " 命中"
	...
	if Global.Player.get_traits().has("Goblin"):
		ToolMessageCreator.add_message("[color=#c0c0c0]", "[color=#90ff50]Goblin's Way![/color] 你获得 [color=#ff8080]+25 最大生命[/color]  [color=#20ff20]+1 速度[/color] [color=#707070]基础[/color]")
