...
func search_is_empty():
	return $TextEdit.text == "" or $TextEdit.text == "search..." or $TextEdit.text == "搜索..."
...
func write_description(trait, button):
	...
	if trait != null:
		...
		if trait.generic == false:
			stringa += "\n\n"
			stringa += translate.element(trait.Element) + " [color=#808080]元素[/color]"
			stringa += "\n\n"
			if Global.Player.abilities.has(trait.title):
				stringa += "[color=#808080]等级 [/color]" + str(Global.Player.abilities[trait.title].Level)
			else:
			...
			match trait.organize:
				"prestige":
					stringa += "[color=#c0c0c0]进阶职业"
				"racial":
					stringa += "[color=#c0c0c0]文化"
				"god":
					stringa += "[color=#c0c0c0]信仰"
				"unit/item":
					stringa += "[color=#c0c0c0]物品"
				"class":
	...
	if trait.generic == false:
		if Global.Player.abilities.has(trait.title):
			stringa += "[color=#c0c0c0]升级[/color] "
		else:
			stringa += "[color=#c0c0c0]学习[/color] "
	...
	if trait.generic == false:
		...
		$PopupLayer / Popup / Button_Learn / text.bbcode_text = "[color=#c0c0c0]进阶职业"
...
func write_elements():
	...
	if Global.Player.ELEMENTS.size() >= 1:
		...
		stringa += "[color=#707070]从 3 个元素中获取能力...[/color]"
