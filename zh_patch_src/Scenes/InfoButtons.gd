...
var zh_font = preload("res://MyFont2.tres")
...
func create_button(info, pos, color):
	...
	apply_zh_font(b.get_node("Label"))
	apply_zh_font(b.get_node("text_icon"))
	
	context.buttons_info.append(b)


func apply_zh_font(label):
	label.add_font_override("normal_font", zh_font)
