		...
		for buff_current in buff.target.Buffs:
			if buff_current.name == buff.name:
				...
				if buff_current.duration > max_duration:
					...
					buff.msg += " (设为上限 " + str(max_duration) + ")"
				...
				form_inherit_msg += " (继承 " + buff_current.name + " 的 +" + str(buff_current.duration) + " 层)"
	...
	if source.object_type == "enemy":
		if StateWorld.day > 0:
			if cycler.get_multi() > 1:
				...
				mod_info.string.append("增幅 +" + str(int(random_increase)))
...
static func message_buff(name, caster, target, duration, color, msg):
	...
	if target == Global.Player and caster == Global.Player:
		name_a = "You"
		name_d = "yourself"
		stringa = "你对自身施加 " + duration_text + name
		stringa += add_msg(msg)
		ToolMessageCreator.add_message(txcolor, stringa)
	elif caster == Global.Player:
		name_a = "You"
		stringa = "你对 " + name_d + " 施加 " + duration_text + name
		stringa += add_msg(msg)
		ToolMessageCreator.add_message(txcolor, stringa)
	elif target == Global.Player:
		name_d = "you"
		stringa = name_a + " 对你施加 " + duration_text + name
		stringa += add_msg(msg)
		ToolMessageCreator.add_message(txcolor, stringa)
	elif caster == target:
		txcolor = "[color=#707070]"
		stringa = name_a + " 对自身施加 " + duration_text + name
		stringa += add_msg(msg)
		ToolMessageCreator.add_message(txcolor, stringa)
	else:
		txcolor = "[color=#707070]"
		stringa = name_a + " 对 " + name_d + " 施加 " + duration_text + name
		stringa += add_msg(msg)
		ToolMessageCreator.add_message(txcolor, stringa)
...
static func add_msg(msg):
	if msg != "":
		msg = "[color=#707070] <- " + textstrip.strip_bbcode(msg) + "[/color]"
	return msg
