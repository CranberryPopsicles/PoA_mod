...
static func check_move(unit, tile_start, tile_end):
	...
	if unit == Global.Player:
			ToolMessageCreator.add_message(txcolor, "你移动了一步")
	...
	if unit_traits.has("KairosVeil") and Global.Enemies.size():
		ToolMessageCreator.add_message("[color=#c07070]", Global.Player.get_traits().KairosVeil.Name + " 触发“原地等待”行动...")
		check_wait(unit)
...
static func check_wait(unit):
	...
	var txcolor = "[color=#c0c0c0]"
	if unit == Global.Player:
			ToolMessageCreator.add_message(txcolor, "你原地等待")
			if calcrange.is_adjacent_enemy(unit, Global.Enemies) == true:
	...
	if unit_traits.has("Kairos_Sash"):
		if Global.Enemies.size() > 0:
			ToolMessageCreator.add_message("[color=#c07070]", unit_traits.Kairos_Sash.Name + " 触发你的“游戏回合”行动...")
			event_turn.check(unit)
	
	if unit_traits.has("Geistform"):
		if Global.Enemies.size() > 0:
			if unit.get_buff_names().has("Crystalform"):
				ToolMessageCreator.add_message("[color=#c07070]", unit_traits.Geistform.Name + " 触发你的“游戏回合”行动...")
				event_turn.check(unit)
	...
	if unit_traits.has("Dreamer"):
		if Global.Enemies.size() > 0:
			...
			ToolMessageCreator.add_message("[color=#c07070]", unit_traits.Dreamer.Name + " 触发你的“游戏回合”行动...")
