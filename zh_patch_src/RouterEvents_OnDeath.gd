...
static func check_effects(dying_unit, killer):
		...
		if dying_unit.type.boss == true:
			if Global.Player.get_traits().has("Eris") and StateWorld.land != "dust":
				...
				if item != null:
					...
					var stringa = "[color=#fff0a0]Struggler! Eris[/color] 强化了你的 " + item.name + " [color=#ffff50]+50%[/color] [color=#ff8030]命中[/color] / [color=#ffa050]精准[/color] / [color=#5050ff]格挡[/color]"
	...
	if dying_unit.object_type == "enemy":
		if Global.Player.is_dead() == false and Global.Player.armor_chest == null and Global.Player.get_traits().has("CrowVisage"):
			...
			ToolMessageCreator.add_message("[color=#c07070]", Global.Player.get_traits().CrowVisage.Name + " 触发“原地等待”行动...")
...
static func death_message(dying_unit, killer):
	...
	var name_d = dying_unit.get_name_color()
	var name_k = killer.get_name_color()
	var txtcolor = "[color=#ffb030]"
	if killer == Global.Player:
		name_k = "你"
		if dying_unit == Global.Player:
			name_d = "自己"
		ToolMessageCreator.add_message(txtcolor, name_k + " 击杀了 " + name_d + "！")
